...
class PrivateUserListSerializer(serializers.ModelSerializer):
    name = serializers.CharField(source="get_name", read_only=True)
    inbox_uid = serializers.SerializerMethodField()
    ...
    def get_inbox_uid(self, object):
        thread = object.thread_set.filter(kind=ThreadKind.PARENT)
        if thread:
